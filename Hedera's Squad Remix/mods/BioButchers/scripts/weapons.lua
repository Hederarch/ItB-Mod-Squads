...
function BB_Science_Rupture:GetSkillEffect(p1,p2)
	...
	for i = 0,2,1 do
	ret:AddDamage(vfx)
	ret:AddBounce(p1,-(i+1)*3)
	ret:AddBounce(p2,-(i+1)*3)
	ret:AddDelay(0.4)
	end
 ...
 BB_Science_Skewer = BB_Science_Rupture:new{
	Name = "Nano Shrapnel",
	Description = "Pulls together local nanites, forming hazardous spikes.",
	Upgrades = 0,
 }
 ...
 function BB_Science_Skewer:GetSkillEffect(p1,p2) 
	local ret = SkillEffect()
	local dir = GetDirection(p2 - p1)
	local damage = SpaceDamage(p2, self.Damage)
	
	if IsTipImage() then
		clear = SpaceDamage(p2,0)
		clear.iTerrain = TERRAIN_ROAD
		ret:AddDamage(clear)
	end
	...
	local vfx = SpaceDamage(p2,0)
	vfx.sAnimation = "SwipeClaw1"
	for i = 0,2,1 do
	ret:AddDamage(vfx)
	ret:AddBounce(p2,-(i+1)*(i+1))
	ret:AddDelay(0.3)
	end
	ret:AddDamage(damage)
